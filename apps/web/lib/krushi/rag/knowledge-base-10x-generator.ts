...
export type Confidence = 'government-verified' | 'expert-reviewed' | 'community-validated';
export type Priority = 'critical' | 'high' | 'medium' | 'low';
export type DocType = 'extension-manual' | 'faq' | 'pest-bulletin' | 'soil-guide' | 'scheme-factsheet' | 'advisory' | 'research-paper' | 'qna' | 'govt-circular' | 'best-practice';
...
export interface KnowledgeRecord {
  ...
  titleHi: string;
  titleMr: string;
  content: string; // Rich markdown with sections
  summary: string;
  summaryHi: string;
  summaryMr: string;
  faq?: { question: string; questionHi: string; questionMr: string; answer: string; answerHi: string; answerMr: string };
  metadata: {
    category: string;
    subcategory?: string;
    crop?: string[];
    ...
    verified: boolean;
    priority: Priority;
    personas: ('farmer' | 'officer' | 'student' | 'ngo' | 'fpo' | 'researcher')[];
    geo?: { lat: number; lng: number; country: string; state: string; district?: string };
    agroZone?: string;
    costRange?: string;
    duration?: string;
  };
  ...
  lastUpdated: string;
  tags: string[];
  keywords: string[]; // 25-40 per record for BM25
  synonyms?: string[];
...
const RICH_TEMPLATES = {
  crop: (crop: string, cropHi: string, cropMr: string, stage: string) => `
...
**⚠️ चेतावनी:** अत्यधिक यूरिया से बचें, केवल मिट्टी जांच के बाद ही डालें।`,
  disease: (crop: string, disease: string, diseaseHi: string) => `
...
**💰 सब्सिडी:** जैविक फफूंदनाशक पर PKVY में 50% अनुदान, KVK से मुफ्त सलाह।`,
  pest: (pest: string, pestHi: string, crop: string) => `
...
function generateSuperKeywords(base: string, baseHi: string, baseMr: string, category: string): string[] {
  ...
  regional.forEach(r => { if (base.includes(r.slice(0,3))) keywords.add(r); });
...
function generateFAQRecords(count: number, startId: number): KnowledgeRecord[] {
  const faqs = [
    {
...
// ─── MAIN 10X GENERATOR ───────────────────────────────────────────────────────
export function generate10xKnowledgeBase(): KnowledgeRecord[] {
  ...
  for (const cat of CATEGORIES_20.slice(0, 4)) {
    const crops = (cat as any).crops as string[];
    const cropsHi = (cat as any).cropsHi as string[];
    const cropsMr = (cat as any).cropsMr as string[];
  ...
  for (const disease of (diseaseCat as any).items) {
  ...
  for (const pest of (pestCat as any).items) {
    for (const crop of pestCrops) {
      const id = `kb_${String(idCounter++).padStart(4, '0')}`;
      allRecords.push({
        id,
  ...
  for (const cat of remainingCats) {
    const items = (cat as any).items || (cat as any).crops || [];
...
export function getDocumentById_10X(id: string) {
  return KNOWLEDGE_BASE_10X.find(d => d.id === id);
}

export function getByCategory_10X(category: string) {
  return KNOWLEDGE_BASE_10X.filter(d => d.metadata.category === category);
}

export function getByState_10X(state: string) {
...
export function getByCrop_10X(crop: string) {
  return KNOWLEDGE_BASE_10X.filter(d => d.metadata.crop?.includes(crop.toLowerCase()));
}

export function getByConfidence_10X(level: Confidence) {

...
export function getDocumentsByCrop(cropId: string) {
  return KNOWLEDGE_BASE.filter(d => (d.metadata.crop as any)?.includes(cropId) || d.metadata.category.includes(cropId));
}
export function getDocumentsByDisease(diseaseId: string) {
  return KNOWLEDGE_BASE.filter(d => (d.metadata.disease as any)?.includes(diseaseId));
}
export function getDocumentsByState(state: string) {
  return KNOWLEDGE_BASE.filter(d => d.metadata.state.includes(state) || d.metadata.state.includes('all'));
}
export function getDocumentsByType(docType: string) {
  return KNOWLEDGE_BASE.filter(d => d.metadata.documentType === docType);
}
export function getDocumentsByConfidence(level: string) {
...
export function getByKeyword(keyword: string) {
